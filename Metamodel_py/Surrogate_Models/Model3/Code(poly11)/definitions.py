...
submodels[submodelName]['p0'] = [0.03, 0., 0.]
submodels[submodelName]['sd'] = [0.05, 0.01, 0.01]
submodels[submodelName]['tableBackgroundColor'] = 'rgba(200, 150, 0, 0.65)'

submodels[submodelName]['fitFunction'] = \
    'p00 + p10*(x/np.mean(x)) + p01*(y/np.mean(y))'
# submodels[submodelName]['fitFunction'] = \
#     'p00 + p10*(x/sum(x)) + p01*(y/sum(y))'


def poly11(xy, p00, p10, p011):
